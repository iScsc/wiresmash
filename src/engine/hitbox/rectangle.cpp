...
Collision Rectangle::checkCollision(Rectangle* r){
    ...
    int ycol2 = (r->getVertices().at(2).first + that_pos->first)
              - (this->relVertices.at(1).first + this_pos->first);

    if(xcol1 > 0 && xcol2 > 0 && ycol1 > 0 && ycol2 > 0){
        coll.collisionVector = std::pair(std::min(xcol1,xcol2), std::min(ycol1, ycol2));
